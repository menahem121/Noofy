...
## How Noofy Was Built with Codex and ChatGPT

I built Noofy in approximately one month, but I believe it could have taken me
years without Codex and ChatGPT. I see myself primarily as a CTO, product
manager, and designer rather than a traditional software developer. I directed
the product, user experience, architecture, priorities, and validation, while
the code itself was generated and implemented through Codex.

I began by designing and building out the complete frontend experience first,
as I usually do. This allowed me to understand the application from the user's
perspective before going deeply into the backend architecture.

Once the main screens and interactions existed, I documented every important
product rule and decision until the expected behavior of each part of Noofy was
clear. I then converted those decisions into detailed specifications covering
both product behavior and technical architecture.

Next, I asked the coding agent to turn those specifications into a structured
backlog, ordered from foundational architectural work through final production
validation. The project began with GPT-5.5 because GPT-5.6 had not yet been
released; once GPT-5.6 became available, I used it for the later stages of the
project.

After the difficult product and design decisions were resolved, I entered what
I call “full-auto vibe coding mode.” Codex implemented the milestones one by
one while I reviewed the results, corrected incorrect assumptions, tested
important paths, and updated the specifications whenever further clarification
was needed. This was an iterative process, not a project created in one prompt
or without human involvement.

ChatGPT helped me reason about product behavior, architecture, edge cases,
documentation, and implementation plans. Codex worked directly with the
repository to implement, test, and refine the code.

AI accelerated the execution dramatically, but the product direction,
prioritization, acceptance decisions, testing, and final responsibility
remained mine throughout the project.
