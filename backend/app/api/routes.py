...
from app.model_inventory import (
...
async def import_models(request: ModelImportRequest, inventory: ModelInventoryServiceDep):
    try:
        ...
        raise HTTPException(status_code=400, detail={"code": "model_import_error", "message": str(exc)}) from exc
...
async def delete_model(model_key: str, inventory: ModelInventoryServiceDep):
    try:
        ...
        raise HTTPException(status_code=400, detail={"code": "model_delete_error", "message": str(exc)}) from exc
...
async def create_model_tag(request: ModelTagCreateRequest, model_tag_store: ModelTagStoreDep):
    try:
        ...
        raise HTTPException(status_code=400, detail={"code": "model_tag_error", "message": str(exc)}) from exc
...
async def update_model_tags(
    ...
    return {"model_key": model_key, "tag_ids": model_tag_store.set_model_tags(model_key, request.tag_ids)}
...
async def start_model_download(
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail={"code": "model_download_error", "message": str(exc)}) from exc
    except KeyError as exc:
        raise HTTPException(status_code=404, detail={"code": "model_download_not_found", "message": str(exc)}) from exc
