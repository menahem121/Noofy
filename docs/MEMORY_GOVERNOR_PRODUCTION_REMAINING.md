...
- immediate cancellable queue handles before runner preparation or memory cleanup
...
| `Not enough memory` was too early in the admission path. | fixed | `runtime/memory`, `runs/orchestrator`, `runs/queue_service`, frontend | Valid run requests receive a cancellable queue handle before preparation. Backend attempts reuse, queueing, safe cleanup, and bounded release polling under that handle, then submits automatically when memory is sufficient. Trusted peak-over-total evidence terminates as `blocked_exceeds_capacity` without cleanup. Classic and Canvas consume the same polled preparation and failure state. | Continue hardware validation of delayed real-model release. |
...
| Cancellation could lose the race during preparation or `adapter.run_workflow(...)`. | fixed | `runs/job_service`, `runs/orchestrator`, `runs/queue_service`, `runtime/runners/supervisor` | The public queue handle exists before runner preparation. Cancellation prevents pre-submission handoff; requests during adapter submission wait for canonical job registration and then cancel through the alias. | No immediate follow-up. |
