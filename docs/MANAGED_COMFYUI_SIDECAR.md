...
Preview launch defaults come from the selected runtime profile / runner launch configuration and participate in runner fingerprints. If a future profile declares a safer lower preview size, the profile value should be used. Isolated workflow runners go further: the full profile `launch_defaults` surface (preview, VRAM mode, attention backend, precision policy, profile env vars) is emitted as real launch args/env from the capsule lock — see "Launch Defaults" in [RUNTIME_ISOLATION_ARCHITECTURE.md](RUNTIME_ISOLATION_ARCHITECTURE.md). The settings-screen VRAM mode above remains a managed-sidecar-only control. External ComfyUI mode is development-only: Noofy may submit per-run `extra_data.preview_method`, but it does not control or claim to control the external server's launch defaults or preview size.

The managed sidecar also sees Noofy's user-visible model storage through a generated `extra-model-paths.yaml` under the runtime store. The configured Noofy Models folder (default `~/Documents/Noofy Models`) is registered as the default category root, and an optional user-connected ComfyUI `models/` folder is registered as a secondary user-owned root for availability checks and reuse. Downloads and imports always land in the Noofy Models folder; Noofy never installs or overwrites models in the external ComfyUI folder, though users may explicitly delete files from that connected folder through the Models page. Noofy must never write models into `third_party/comfyui/`. See [MODEL_RESOLUTION_AND_DOWNLOADS.md](MODEL_RESOLUTION_AND_DOWNLOADS.md).
