...
- `deleteModelFile(modelKey)`: delete a regular model file only when the backend resolves it inside a configured model root. Files in Noofy Models are deletable only when Noofy recorded them as imported or downloaded by Noofy; files in the connected external ComfyUI models folder are deletable by explicit user action. Arbitrary user-owned files placed inside Noofy Models, engine-visible references outside configured roots, and missing workflow requirements are not deletable through this operation.
